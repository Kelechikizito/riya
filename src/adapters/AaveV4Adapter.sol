...
contract AaveV4Adapter is ReentrancyGuard {
    ...
    function harvest() external returns (uint256 assets) {
        ...
        if (available < I_MIN_HARVEST) revert AaveV4Adapter__HarvestBelowMinimum(available, I_MIN_HARVEST);

        (, assets) = I_SPOKE.withdraw(I_RESERVE_ID, available, address(this));
