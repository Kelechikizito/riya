...
contract AaveV4Adapter is IYieldAdapter, ReentrancyGuard {
    ...
    constructor(address vault, IAaveV4Spoke spoke, uint256 reserveId, uint256 minHarvest) {
        ...
        I_VAULT = vault; // @to-do: write the vault interface for later
     ...
     */
    function deposit(uint256 amount) external nonReentrant onlyVault returns (uint256 assets) {
     ...
     */
    function withdraw(uint256 amount, address to) external nonReentrant onlyVault returns (uint256 assets) {
        return assets = _withdraw(amount, to);
    }

    /**
     * @notice Moves everything Aave holds above principal into the vault.
     * @dev Permissionless — anyone may pay the gas to retire someone else's debt, and the yield can only ever go to the vault. The `I_MIN_HARVEST` floor stops the
