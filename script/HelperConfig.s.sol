...
contract HelperConfig is Script {
    ...
    function getMainnetConfig() public pure returns (NetworkConfig memory) {
        return NetworkConfig({
            spoke: MAINNET_SPOKE, reserveId: MAINNET_USDC_RESERVE_ID, minHarvest: MIN_HARVEST, minDeposit: MIN_DEPOSIT
        });
    }
     ...
     * @dev Not `view`, because it deploys. Runs from the constructor and so outside any
     *      broadcast, which keeps the mocks out of any transaction a script would send.
     *      Guarded against re-entry, which would strand the first pair.
     *
     *      Thresholds match the live chains: a local run under a softer floor proves
     *      nothing about the real one.
     */
    function getAnvilConfig() public returns (NetworkConfig memory) {
        if (activeNetworkConfig.spoke != address(0)) return activeNetworkConfig;

        MockUSD mockUsd = new MockUSD();
        MockAaveSpoke mockSpoke = new MockAaveSpoke();
        uint256 reserveId = mockSpoke.listReserve(address(mockUsd));

        return NetworkConfig({
            spoke: address(mockSpoke), reserveId: reserveId, minHarvest: MIN_HARVEST, minDeposit: MIN_DEPOSIT
        });
    }
}
